...
package br.gov.lexml.jshortcut;
 ...
 * 
 * The native library (jshortcut.dll) is loaded when JShellLink is first loaded.
 * By default, JShellLink first looks for the native library in the PATH, using
 * System.loadLibrary. If the native library is not found in the PATH,
 * JShellLink then looks through each directory in the CLASSPATH (as determined
 * by the value of the system property java.class.path). If an entry in the
 * CLASSPATH is a jar file, then JShellLink looks for the native library in the
 * directory containing that jar file. The application can override this
 * behavior and force JShellLink to look for the native library in a specific
 * directory by setting the system property JSHORTCUT_HOME to point to that
 * directory. This property must be set before the JShellLink class is loaded.
...
public class JShellLink {
	...
	static {
		try {
			...
			String libname = "jshortcut_" + System.getProperty("os.arch")
					+ ".dll";
	 ...
	 *            <ul>
	 *            <li>common_desktopdirectory - the All Users desktop folder
	 *            (not on earlier Windows)
	 *            <li>common_programs - the All Users programs (not on earlier
	 *            Windows)
	...
	/**
	 * Set the path for this shortcut. If the working directory is null, this
	 * call also sets the working directory to the parent of the given path.
	 */
	...
	/**
	 * Load a shortcut. The native code reads the following variables from this
	 * object: folder, name. The native code fills in the following variables in
	 * this object: description, path, workingDirectory, iconLocation,
	 * iconIndex.
	 */
	private native boolean nLoad();

	/**
	 * Save a shortcut. The native code reads the following variables from this
	 * object: folder, name, description, path, workingDirectory, iconLocation,
	 * iconIndex.
	 */
